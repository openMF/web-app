...
export class CurrencyComponent implements OnInit, AfterViewInit {
  ...
  async changeLiveRate() {
    this.useLiveRate = !this.useLiveRate;
    //load and update live rates here
    const response = await fetch('https://api.exchangerate-api.com/v4/latest/USD');
    const data = await response.json();
    this.loadingLiveRate = false;
    this.liveRate = data.rates.UGX
    this.http.post('/update-live-rate', this.liveRate);
  }

  updateRate(i: number) {
    //triggered from check box
    //if live rate is checked then manual rate should not be displayed
  }
