...
export type McpStreamEventType = 'token' | 'thinking' | 'tool_call' | 'action_card' | 'suggest' | 'done' | 'error';
...
export interface McpStreamEvent {
  ...
  summary?: string;
  /**
   * What this step is called in banking terms, from the gateway's tool manifest.
   *
   * <p>Never generated by a model. A step log is part of the record of what was done on a
   * client's account, and a record whose wording changes between runs is not much of a record.
   */
  toolLabel?: string;
  /** How long the step took, once it has finished. */
  durationMs?: number;
  /** Present for 'thinking' events. */
  thinkingPhase?: 'start' | 'delta' | 'end';
  /** A fragment of the model's working notes. */
  thinking?: string;
  /** How long the model spent writing them, on the closing event. */
