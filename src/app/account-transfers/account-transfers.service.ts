...
export class AccountTransfersService {
  ...
  getAccountByNumber(accountNumber: string, currency: string): Observable<any> {
    ...
    return this.http.post(`${environment.vNextApiUrl}${environment.vNextApiVersion}${environment.vNextApiProvider}/participant`, JSON.stringify(payload), { headers }).pipe(
      switchMap((participant: any) => {
      const body = JSON.stringify({...payload, ownerFspId: participant.fspId	});
      return this.http.post(`${environment.vNextApiUrl}${environment.vNextApiVersion}${environment.vNextApiProvider}/partyinfo`, body, { headers });
  })
  );
  }
  ...
  sendInterbankTransfer(body: any): Observable<any> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.post(`${environment.vNextApiUrl}${environment.vNextApiVersion}${environment.vNextApiProvider}/executetransfer`, body, { headers });
  }
