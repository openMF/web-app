...
export class MakeAccountTransfersComponent implements OnInit, AfterViewInit {
  ...
  setParams() {
    ...
    switch (this.accountType) {
      ...
      case 'interbank':
        ...
        this.balance = this.router.getCurrentNavigation()?.extras?.state?.balance || 0;
        break;
      default:
  ...
  ngOnInit() {
    ...
    if (!this.interbank) {
      ...
      // Para interbank, crear un formulario vacío inicialmente
  ...
  createMakeAccountInterbankTransferForm(account: any) {
    ...
    console.log('Creating interbank form with account:', account);

    // Usar un monto por defecto mayor a 0
    const defaultAmount = this.accountTransferTemplateData?.transferAmount > 0 
      ? this.accountTransferTemplateData.transferAmount 
      : 1; // Usar 1 como valor por defecto en lugar de 0

    this.makeAccountTransferForm = this.formBuilder.group({
      toBank: [account.destinationFspId || '', Validators.required],
      toClientId: [(account.firstName || account.firsName || '') + ' ' + (account.lastName || ''), Validators.required],
      toAccountType: ['Saving Account', Validators.required],
      toAccountId: [account.partyId || '', Validators.required],
      transferAmount: [
        defaultAmount,
        [
          Validators.required,
      ...
      transferDate: [this.settingsService.businessDate, Validators.required],
      transferDescription: ['Transferencia interbancaria', Validators.required] // Valor por defecto
    });

    console.log('Form created with values:', this.makeAccountTransferForm.value);
    console.log('Form valid:', this.makeAccountTransferForm.valid);
  ...
  makeTransfer() {
    ...
    // Verificar que toClientId tenga el formato correcto
  ...
  makeInterbankTransfer() {
    ...
    // Verificar que el formulario tenga los valores necesarios
    if (!this.makeAccountTransferForm.valid) {
      console.error('Formulario interbancario no es válido');
      this.isLoading = false;
      return;
    }
    ...
    this.accountTransfersService.sendInterbankTransfer(JSON.stringify(payload)).subscribe(
      ...
      (error) => {
        console.error('Error en transferencia interbancaria:', error);
        this.isLoading = false;
      }
    );
  }
