...
export class MakeAccountTransfersComponent implements OnInit, AfterViewInit {
  ...
  createMakeAccountTransferForm() {
    this.makeAccountTransferForm = this.formBuilder.group({
      ...
      'transferAmount': [this.accountTransferTemplateData.transferAmount, [Validators.required, Validators.min(0.01), this.amountExceedsBalanceValidator.bind(this)]],
  ...
  createMakeAccountInterbankTransferForm(account:any){
   /* --> */ this.makeAccountTransferForm = this.formBuilder.group({
      'toBank': [{value: account.sourceFspId, disabled: true}, Validators.required],
      //'toOfficeId': ['', Validators.required],
      'toClientId': [{value: account.firsName + ' ' + account.lastName, disabled: true}, Validators.required],
      'toAccountType': [{value: 'Saving Account', disabled: true}, Validators.required],
      'toAccountId': [{value: account.partyId, disabled: true}, Validators.required],
      'transferAmount': [this.accountTransferTemplateData.transferAmount, [Validators.required, Validators.min(0.01), this.amountExceedsBalanceValidator.bind(this)]],
      'transferDate': [this.settingsService.businessDate, Validators.required],
      'transferDescription': ['', Validators.required],
    });
  ...
  makeInterbankTransfer(){
    ...
    this.accountTransfersService.sendInterbankTransfer(JSON.stringify(payload)).subscribe((trnsfr) => {
  ...
  searchAccountByNumber() {
    this.isLoading = true;
    this.accountTransfersService.getAccountByNumber(this.phoneAccount, this.accountTransferTemplateData.currency.code).subscribe((acc) => {
