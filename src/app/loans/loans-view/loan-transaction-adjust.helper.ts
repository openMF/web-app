 ...
 *
 * Working Capital loans expose the same command on their own resource
 * (`POST /working-capital-loans/{loanId}/transactions/{transactionId}`) with
 * narrower gates, kept in a separate set below; their reversals keep going
 * through the dedicated `undo` command.
 */
...
/**
 * True when a Working Capital transaction can be re-submitted with a new date,
 * amount and payment details through the Working Capital adjust command.
 * @param transactionType Type of the transaction
 * @param alreadyReversed Whether the transaction is already reversed
 */
export function canAdjustWorkingCapitalTransaction(
