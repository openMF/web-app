...
describe('LoanTransactionAdjustHelper', () => {
  ...
  describe('Working Capital', () => {
    it('can re-submit a repayment with a new amount', () => {
      expect(canAdjustWorkingCapitalTransaction(transactionType(2), false)).toBe(true);
    });

    it('offers no adjustment once the repayment is reversed', () => {
      expect(canAdjustWorkingCapitalTransaction(transactionType(2), true)).toBe(false);
    });
