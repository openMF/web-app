...
describe('LoanGuarantorsTabComponent', () => {
  ...
  function createComponent(
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        // Echo the last key segment, so assertions read like the English copy.
        { provide: TranslateService, useValue: { instant: jest.fn((key: string) => key.split('.').pop()) } }
      ]
    });
  ...
  it('asks for delete confirmation with translated copy', () => {
    const component = createComponent();

    component.deleteGuarantor(7);

    expect(dialogStub.open).toHaveBeenCalledWith(DeleteDialogComponent, {
      data: { deleteContext: 'Guarantor ID: 7' }
    });
  });
