...
export class ViewTransactionComponent extends LoanAccountActionsBaseComponent implements OnInit {
  ...
  constructor() {
    ...
    this.route.data.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((data: { loansAccountTransaction: any }) => {
      ...
      // Working Capital keeps its own reversal rules; the gates below describe
      // the Term Loan adjust command, which Working Capital does not use.
      this.allowEdition = !this.isWorkingCapital && canAdjustLoanTransaction(this.transactionType, alreadyReversed);
