...
describe('ViewTransactionComponent', () => {
  ...
  /**
   * Builds a Working Capital transaction, which dates itself with
   * `transactionDate` and carries only the `reversed` flag.
   * @param typeId Loan transaction type id
   * @param overrides Extra fields
   */
  function workingCapitalTransaction(typeId: number, overrides: any = {}): any {
    const { date, manuallyReversed, ...rest } = transaction(typeId);
    return { ...rest, transactionDate: date, reversed: false, ...overrides };
  }
  ...
  describe('action availability', () => {
    ...
    it('offers the adjustment on a Working Capital repayment', () => {
      const component = createComponent(workingCapitalTransaction(2), { isWorkingCapital: true });
    ...
    it('offers no adjustment on an already reversed Working Capital repayment', () => {
      const component = createComponent(workingCapitalTransaction(2, { reversed: true }), {
